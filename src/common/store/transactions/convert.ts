...
export const HEFineTransactionToHiveTransactions = (
  r: Array<HEFineTransaction>
) => {
  try {
    const mapped: Transaction[] = r.map(
      (x: HEFineTransaction): AuthorReward | CurationReward => {
...
export function HETokensStakeToTransferToVesting(
  het: HETokensStake
): TransferToVesting {
...
export function HEToHMarketSell(
  het: HEMarketSell,
