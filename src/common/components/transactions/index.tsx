    ...
    render() {
        ...
        if (tr.type === "author_reward" || tr.type === "comment_benefactor_reward") {
            ...
            numbers = (
                <>
                    {tr.he_payout &&
                        <span className="number">{tr.he_payout}</span>
                    }
                    {hbd_payout.amount > 0 && (
        ...
        if (tr.type === "claim_reward_balance") {
            ...
            numbers = (
                <>
                    {tr.reward_he &&
                        <span className="number">{tr.reward_he}</span>
                    }
                    {reward_hbd.amount > 0 && (
        ...
        if (tr.type === 'market_placeOrder') {
            ...
            icon = commentSvg;
            numbers = (
                <span className='number'>{tr.quantityLocked}</span>
                );
            if (tr.price)
                details = (<><span>{tr.orderType}</span> <span className='number'>@{tr.price}</span></>);
            else
                details = tr.orderType;
        }
        ...
        if (tr.type === 'market_sell') {
            flag = true;
            icon = arrowRightSvg;
            numbers = <>Sold <span className="number">{tr.quote}</span> for <span className="number">{tr.base}</span></>
        }
