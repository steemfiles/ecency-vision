...
export const prepareVotes = (
  entry: Entry,
  votes: Vote[],
  hiveEngineTokensProperties: TokenPropertiesMap | undefined
): Vote[] => {
  let totalHivePayout =
  ...
  if (he && hiveEngineTokensProperties) {
    for (const token in he) {
      ...
      if (complete_payout_value > 0) {
        const tokenAmount =
          complete_payout_value * Math.pow(10, -postTokenRewardInfo.precision);
...
export class EntryVotesDetail extends BaseComponent<DetailProps, DetailState> {
  ...
  setVotes = (data: Vote[]) => {
    ...
    this.stateSet({
      votes: prepareVotes(entry, data, hiveEngineTokensProperties),
      loading: false,
      originalVotes: prepareVotes(entry, data, hiveEngineTokensProperties),
    });
  };
