...
export class WalletHiveEngine extends BaseComponent<Props, State> {
    ...
    handleFineTransactions(fts: Array<HEFineTransaction>) {
        ...
        const ntxs = [...HEFineTransactionToHiveTransactions(fts),...list].sort( (a,b) => (a.timestamp > b.timestamp ? -1 : 1) );
           this.stateSet({transactions: {list: ntxs, loading:false, group: transactions.group}});
    }
    handleCoarseTransactions(cts: Array<HECoarseTransaction>) {
        {
        ...
        const {transactions} = this.state;
        const {list} = transactions;
        const ntxs = [...txs, ...list].sort( (a,b) => (a.timestamp > b.timestamp ? -1 : 1) );
           this.stateSet({transactions: {list: ntxs, loading:false, group: transactions.group}});
    }
    fetchHETransactions = (name : string, group?: OperationGroup | "") => {
        const {transactions} = this.state;
        console.log("fetchHETransactions called.");
        this.stateSet({transactions: {list: transactions.list, loading:true, group:group || ""}});
        if (!group || group === "rewards")
            getFineTransactions(name, 10, 0).then(this.handleFineTransactions.bind(this)).catch(e => {
                console.log(e);
        });
        if (!group || group !== "rewards")
            getCoarseTransactions(name, 40, 0).then(this.handleCoarseTransactions.bind(this));
    }
