...
import {HEFineTransactionToHiveTransactions, HEToHTransaction}
	from "../../store/transactions/convert";
import {OperationGroup, Transactions, Transaction, HEFineTransaction,
	HECoarseTransaction} from "../../store/transactions/types";
...
import {is_FullHiveEngineAccount, is_not_FullHiveEngineAccount,
	FullHiveEngineAccount, getAccountHEFull, TokenStatus, getFineTransactions,
getCoarseTransactions} from "../../api/hive-engine";
...
export class WalletHiveEngine extends BaseComponent<Props, State> {
	...
	keepTransaction(group?: (OperationGroup | ""), tx : Transaction) {
		...
		switch (group) {
			case 'transfers':
				return (['transfers', 'transfer_to_vesting',
					'transfer_to_saving', 'withdraw_vesting',
					'proposal-pay', "cancel_transfer_from_savings",
					"producer_reward", 'tokens_issue', "claim_reward_balance"].includes(tx.type));
			case 'interests':
				return tx.type === 'interest';
			case 'stake-operations':
				return (['tokens_CancelUnstake', "tokens_undelegateDone",
					"tokens_delegate",	 "tokens_unstakeStart",
				"return_vesting_delegation"]).includes(tx.type);
			case 'rewards':
				return tx.type.endsWith('_reward');
			case  "market-orders":
				return tx.type.startsWith('market_') || tx.type === "fill_order";
			case "stake-operations":
				return "tokens_stake,withdraw_vesting,return_vesting_delegation,tokens_unstakeStart,tokens_CancelUnstake,tokens_undelegateDone,tokens_delegate,tokens_undelegateStart,".split(/,/).includes(tx.type)
		}
		// @ts-ignore
		return false;
	}
	...
	handleFineTransactions(fts: Array<HEFineTransaction>) {
		...
		this.stateSet({transactions: {list: ntxs, loading:false, group: transactions.group}});
	}
	handleCoarseTransactions(group?: OperationGroup | "", cts: Array<HECoarseTransaction>) {
		{
			const utxes = cts.filter( x => !["market_cancel", "market_sell", "market_placeOrder", "tokens_unstakeStart",
			"tokens_transfer", "tokens_issue", "tokens_cancelUnstake", "tokens_delegate",
			"tokens_stake", "tokens_undelegateDone",  "tokens_undelegateStart"].includes(x.operation) );
			if (utxes.length)
				console.log("coarse trxes with unknown type", utxes);
		}
		const {transactions} = this.state;
		try {
			console.log("H");
			const txs: Array<Transaction> =  cts.map( t => HEToHTransaction(t) ); /*cts.filter( x => !["market_cancel", "market_sell", "market_placeOrder", "tokens_unstakeStart",
			...
			const ftxs: Array<Transaction> = [...txs, ...list]
				...
				.sort(this.compareTransactions);
			this.stateSet({transactions: {list: ftxs, loading:false, group: transactions.group}});
			console.log("R");
		} catch (e) {
			console.log(e);
			this.stateSet({transactions: {list: transactions.list, loading:false, group: transactions.group}});
		}
	}
	...
	render() {
		...
		const {claiming, claimed, transfer, transferAsset, transferMode, converting, transactions} = this.state;
		if (!account.__loaded) {
			return null;
		}
				...
				<div className="wallet-hive">
					<div className="wallet-main">
						<div className="wallet-info">
							...
							{TransactionList({dynamicProps, fetchTransactions: this.fetchHETransactions.bind(this), history: this.props.history,
											transactions: this.state.transactions, account: this.props.account})}
						</div>
