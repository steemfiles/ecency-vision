...
class EntryPage extends BaseComponent<Props, State> {
  ...
  detect = () => {
    const infoCard: HTMLElement | null =
      document.getElementById("avatar-fixed");
  ...
  ensureEntry = async () => {
    const { match, addEntry, updateEntry, addCommunity, activeUser, global } =
      this.props;
  ...
  render() {
    ...
    const isHidden : boolean = (tokenScotPost && tokenScotPost.pending_token === 0 && tokenScotPost.active_votes.length > 0) || false;
    // There is no list of muted user in neither the info or the config of the token...
    const isMuted : boolean = (entry?.he && entry?.he[LIQUID_TOKEN_UPPERCASE].muted) || false;
                  ...
                  <>
                    ...
                    {activeUser &&
                      Comment({
                        ...
                        defText:
                          ls.get(
                            `reply_draft_${entry.author}_${entry.permlink}`
                          ) || "",
